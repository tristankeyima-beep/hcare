...
- `llmOutput: String(JSON) <= 生成运动干预方案总述.text`

## 入参

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `llmOutput` | String(JSON) | 是 | 当前 yml 实际绑定字段，传入节点4 LLM 的整段 JSON 文本。 |
| `llmText` | String(JSON) | 否 | 兼容字段；当 `llmOutput` 为空时读取该字段。 |
| `planName` | String | 否 | 兼容散字段；当前 yml 不按该字段绑定。 |
| `planTitle` | String | 否 | 兼容散字段；当前 yml 不按该字段绑定。 |
| `planSummary` | String | 否 | 兼容散字段；当前 yml 不按该字段绑定。 |
| `executionPoints` | String | 否 | 兼容散字段；当前 yml 不按该字段绑定。 |
...
| `planHeaderText` | String(JSON) | `planHeader` 的 JSON 字符串版本。当前 yml 后续实际传该字段。 |
