...
- `planHeader: String(JSON) <= 将运动方案总述结构化.planHeaderText`
- `groups: String(JSON Array) <= 将运动方案分组清单结构化.groupsText`
- `groupPlan: String(JSON Array) <= 将运动方案分组清单结构化.groupPlanText`

## 入参

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `planHeader` | String(JSON) | 否 | 当前 yml 实际绑定字段，节点4 `planHeaderText` 输出，包含 `planName`、`planTitle`、`planSummary`、`executionPoints`。 |
| `groupPlan` | String(JSON Array) | 否 | 当前 yml 实际绑定字段，节点3 `groupPlanText` 输出，用于按规划顺序排序 groups。 |
| `groups` | String(JSON Array) | 否 | 当前 yml 实际绑定字段，节点3 `groupsText` 输出，包含最终方案条目。 |
