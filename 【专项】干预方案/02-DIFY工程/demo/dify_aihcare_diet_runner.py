...
DEFAULT_SOURCE_FILE = PROJECT_ROOT / "饮食方案" / "DIFY工程-AI干预方案-饮食方案" / "测试数据" / "【入参】饮食方案工作流测试入参.json"
DEFAULT_API_BASE = "https://dify.hzmarvel.com/v1"
DEFAULT_RESPONSE_MODE = "streaming"
DEFAULT_USER = "dify-aihcare-diet-chatflow-test"
DEFAULT_QUERY = "生成干预方案"
...
def build_run_command(case_dir, env_name=DEFAULT_ENV_NAME, api_base=DEFAULT_API_BASE):
    ...
    return (
        ...
        f"{env_api_key_name(env_name)}=\"app-***\" python3 dify_aihcare_diet_runner.py run "
...
def normalize_inputs(raw_input):
    ...
    normalized["planType"] = normalized.get("planType") or "diet"
...
def prepare_input_file(input_path, output_root=None, now=None, env_name=DEFAULT_ENV_NAME):
    ...
    case_dir = output_root / f"{safe_patient}_饮食方案_{compact_timestamp(now)}"
...
def extract_final_plan(answer):
    ...
    if start < 0 or end < 0:
        return None, "未找到 FINAL_PLAN_JSON 标签"
    text = answer[start + len(start_tag):end].strip()
    try:
        return json.loads(text), None
    except json.JSONDecodeError as exc:
        return None, f"FINAL_PLAN_JSON 解析失败：{exc}"
...
def sum_number(items, key):
    return sum(normalize_number(item.get(key)) for item in items if isinstance(item, dict))


def validate_final_plan(plan):
    summary = {"errors": [], "weeklyDaysCount": 0, "mismatches": [], "zeroCarbFoods": []}
    if not isinstance(plan, dict):
        summary["errors"].append("finalPlan 必须是对象")
        return summary
    groups = plan.get("groups")
    if not isinstance(groups, list):
        summary["errors"].append("groups 必须是数组")
        return summary
    weekly = next((group for group in groups if isinstance(group, dict) and group.get("groupType") == "weeklyMealPlan"), None)
    if not weekly:
        summary["errors"].append("未找到 weeklyMealPlan")
        return summary
    items = weekly.get("items") or []
    summary["weeklyDaysCount"] = len(items) if isinstance(items, list) else 0
    if summary["weeklyDaysCount"] != 7:
        summary["errors"].append(f"weeklyMealPlan 当前包含 {summary['weeklyDaysCount']} 天，期望 7 天")
    for item in items if isinstance(items, list) else []:
        if not isinstance(item, dict):
            continue
        day = item.get("day", "")
        meals = item.get("meals") or []
        daily_sums = {field[0]: 0 for field in NUTRITION_FIELDS}
        for meal in meals if isinstance(meals, list) else []:
            ...
            for food in foods if isinstance(foods, list) else []:
                if not isinstance(food, dict):
                    continue
                if normalize_number(food.get("carbsG")) == 0:
                    summary["zeroCarbFoods"].append({
                        "day": day,
                        "meal": meal_name,
                        "name": food.get("name", ""),
                        "amountG": food.get("amountG", ""),
                        "kcal": food.get("kcal", ""),
                        "proteinG": food.get("proteinG", ""),
                        "fatG": food.get("fatG", ""),
                        "carbsG": food.get("carbsG", ""),
                    })
            for food_key, meal_key, _daily_key in NUTRITION_FIELDS:
                expected = sum_number(foods, food_key)
                actual = normalize_number(meal.get(meal_key))
                if actual != expected:
                ...
                daily_sums[food_key] += actual
        for food_key, _meal_key, daily_key in NUTRITION_FIELDS:
            actual = normalize_number(item.get(daily_key))
            expected = daily_sums[food_key]
            if actual != expected:
                summary["mismatches"].append({
                    "level": "day",
                    "day": day,
...
def render_meal_plan_table(final_plan):
    groups = final_plan.get("groups") if isinstance(final_plan, dict) else []
    weekly = next((group for group in groups if isinstance(group, dict) and group.get("groupType") == "weeklyMealPlan"), None)
    if not weekly:
        return "<p class=\"empty\">没有 7 天菜谱。</p>"
    rows = []
    for item in weekly.get("items") or []:
        if not isinstance(item, dict):
            continue
        rows.append(
            "<tr>"
            f"<td>第{escape(str(item.get('day', '')))}天</td>"
            f"<td>{escape(str(item.get('dailyTotalKcal', '')))}</td>"
            f"<td>{escape(str(item.get('dailyTotalProteinG', '')))}</td>"
            f"<td>{escape(str(item.get('dailyTotalFatG', '')))}</td>"
            f"<td>{escape(str(item.get('dailyTotalCarbsG', '')))}</td>"
            f"<td>{escape(str(item.get('focusPoint', '')))}</td>"
            "</tr>"
        )
    return "<table><thead><tr><th>天数</th><th>kcal</th><th>蛋白</th><th>脂肪</th><th>碳水</th><th>重点</th></tr></thead><tbody>" + "".join(rows) + "</tbody></table>"


def weekly_items(final_plan):
    weekly = get_weekly_group(final_plan)
    if not weekly:
        return []
...
def meal_food_count(meal):
    foods = meal.get("foods") if isinstance(meal, dict) and isinstance(meal.get("foods"), list) else []
    return len(foods)


def macro_percent(kcal, protein, fat, carbs):
    total = normalize_number(kcal)
    if not total:
        return {"protein": "-", "fat": "-", "carbs": "-"}
    return {
        "protein": f"{protein * 4 / total * 100:.0f}%",
        "fat": f"{fat * 9 / total * 100:.0f}%",
        "carbs": f"{carbs * 4 / total * 100:.0f}%",
    }


def sum_day_nutrition(items):
    totals = {"kcal": 0, "proteinG": 0, "fatG": 0, "carbsG": 0, "meals": 0, "foods": 0}
    for item in items:
        if not isinstance(item, dict):
            continue
        totals["kcal"] += normalize_number(item.get("dailyTotalKcal"))
        totals["proteinG"] += normalize_number(item.get("dailyTotalProteinG"))
        totals["fatG"] += normalize_number(item.get("dailyTotalFatG"))
        totals["carbsG"] += normalize_number(item.get("dailyTotalCarbsG"))
        meals = item.get("meals") if isinstance(item.get("meals"), list) else []
        totals["meals"] += len(meals)
        totals["foods"] += sum(meal_food_count(meal) for meal in meals if isinstance(meal, dict))
    return totals


def render_nutrition_badges(kcal, protein, fat, carbs):
    percents = macro_percent(kcal, protein, fat, carbs)
    return (
        "<div class=\"nutrition-badges\">"
        f"<span><b>{escape(str(kcal))}</b> kcal</span>"
        f"<span><b>{escape(str(protein))}g</b> 蛋白 · {escape(percents['protein'])}</span>"
        f"<span><b>{escape(str(fat))}g</b> 脂肪 · {escape(percents['fat'])}</span>"
        f"<span><b>{escape(str(carbs))}g</b> 碳水 · {escape(percents['carbs'])}</span>"
        "</div>"
    )


def render_day_summary_metrics(item):
    meals = item.get("meals") if isinstance(item.get("meals"), list) else []
    return (
        "<div class=\"day-summary-metrics\" aria-label=\"每日摄入\">"
        "<span class=\"summary-label\">每日摄入</span>"
        f"<span><b>{escape(str(item.get('dailyTotalKcal', 0)))}</b> kcal</span>"
        f"<span>P {escape(str(item.get('dailyTotalProteinG', 0)))}g</span>"
        f"<span>F {escape(str(item.get('dailyTotalFatG', 0)))}g</span>"
        f"<span>C {escape(str(item.get('dailyTotalCarbsG', 0)))}g</span>"
        f"<span>{len(meals)} 餐</span>"
        "</div>"
    )


def render_weekly_nutrition_summary(final_plan):
    items = weekly_items(final_plan)
    if not items:
        return "<p class=\"empty\">没有 7 天菜谱总计。</p>"
    totals = sum_day_nutrition(items)
    avg_kcal = round(totals["kcal"] / len(items)) if items else 0
    avg_carbs = round(totals["carbsG"] / len(items)) if items else 0
    return (
        "<section class=\"meal-subsection\">"
        "<h4>7天总计</h4>"
        f"{render_nutrition_badges(totals['kcal'], totals['proteinG'], totals['fatG'], totals['carbsG'])}"
        "<div class=\"dietitian-notes\">"
        f"<span>日均 {escape(str(avg_kcal))} kcal</span>"
        f"<span>日均碳水 {escape(str(avg_carbs))}g</span>"
        f"<span>共 {escape(str(totals['meals']))} 餐</span>"
        f"<span>共 {escape(str(totals['foods']))} 个食物项</span>"
        "</div>"
        "</section>"
    )


def render_food_table(foods):
    if not isinstance(foods, list) or not foods:
        return "<p class=\"empty\">没有食物明细。</p>"
    rows = []
    for food in foods:
        if not isinstance(food, dict):
            continue
        rows.append(
            "<tr>"
            f"<td>{escape(str(food.get('name', '')))}</td>"
            f"<td>{escape(str(food.get('amountG', '')))}g</td>"
            f"<td>{escape(str(food.get('kcal', '')))}</td>"
            f"<td>{escape(str(food.get('proteinG', '')))}</td>"
            f"<td>{escape(str(food.get('fatG', '')))}</td>"
            f"<td>{escape(str(food.get('carbsG', '')))}</td>"
            "</tr>"
        )
    return (
        "<div class=\"table-scroll\"><table class=\"food-table\">"
        "<thead><tr><th>食物</th><th>份量</th><th>kcal</th><th>蛋白g</th><th>脂肪g</th><th>碳水g</th></tr></thead>"
        f"<tbody>{''.join(rows)}</tbody></table></div>"
    )


def render_meal_card(meal):
    foods = meal.get("foods") if isinstance(meal.get("foods"), list) else []
    scene = meal.get("mealScene")
    scene_html = f"<span class=\"meta\">{escape(str(scene))}</span>" if scene else ""
    return (
        "<details class=\"meal-card\" open>"
        "<summary>"
        f"<span><strong>{escape(str(meal.get('mealName', '餐次')))}</strong>{scene_html}</span>"
        f"<em>{escape(str(meal.get('mealTotalKcal', 0)))} kcal · P{escape(str(meal.get('mealTotalProteinG', 0)))} F{escape(str(meal.get('mealTotalFatG', 0)))} C{escape(str(meal.get('mealTotalCarbsG', 0)))}</em>"
        "</summary>"
        "<h5>食物明细</h5>"
        f"{render_food_table(foods)}"
        "</details>"
    )


def render_daily_meal_plan(final_plan):
    items = weekly_items(final_plan)
    if not items:
        return "<p class=\"empty\">没有每天菜谱。</p>"
    day_blocks = []
    for item in items:
        if not isinstance(item, dict):
            continue
        meals = item.get("meals") if isinstance(item.get("meals"), list) else []
        open_attr = " open" if normalize_number(item.get("day")) == 1 else ""
        day_blocks.append(
            f"<details class=\"day-card\"{open_attr}>"
            "<summary>"
            f"<span><strong>第{escape(str(item.get('day', '')))}天</strong><em>{escape(str(item.get('content', '')))}</em></span>"
            f"{render_day_summary_metrics(item)}"
            "</summary>"
            "<div class=\"day-total\">"
            "<h4>每天总计</h4>"
            f"{render_nutrition_badges(item.get('dailyTotalKcal', 0), item.get('dailyTotalProteinG', 0), item.get('dailyTotalFatG', 0), item.get('dailyTotalCarbsG', 0))}"
            f"<p>{escape(str(item.get('focusPoint', '')))}</p>"
            "</div>"
            "<div class=\"meal-list\"><h4>每餐明细</h4>"
            f"{''.join(render_meal_card(meal) for meal in meals if isinstance(meal, dict))}"
            "</div>"
            "</details>"
        )
    return "<section class=\"meal-subsection\"><h4>每天总计 / 每餐明细</h4>" + "".join(day_blocks) + "</section>"


def render_dietitian_meal_plan(final_plan):
    return (
        "<div class=\"dietitian-meal-plan\">"
        f"{render_weekly_nutrition_summary(final_plan)}"
        f"{render_daily_meal_plan(final_plan)}"
        "</div>"
    )
...
def render_validation(summary):
    ...
    parts = [
        f"<p>7天菜谱天数：<strong>{escape(str(summary.get('weeklyDaysCount', 0)))}</strong></p>",
        f"<p>营养汇总不一致：<strong>{len(summary.get('mismatches') or [])}</strong></p>",
        f"<p>碳水为0的食物：<strong>{len(summary.get('zeroCarbFoods') or [])}</strong></p>",
    ]
    if summary.get("errors"):
        parts.append("<pre>" + escape(summary["errors"]) + "</pre>")
    if summary.get("mismatches"):
        parts.append("<h3>汇总不一致</h3><pre>" + escape(summary["mismatches"]) + "</pre>")
    if summary.get("zeroCarbFoods"):
        parts.append("<h3>carbsG=0 食物</h3><pre>" + escape(summary["zeroCarbFoods"]) + "</pre>")
    return "".join(parts)
...
def render_input_quality(case_record):
    ...
    cards = [
        ...
        ("饮食记录", quality["counts"]["饮食记录"], "近一年 diet records"),
...
def meal_plan_completeness(final_plan):
    weekly = get_weekly_group(final_plan)
    result = {"missingMeals": [], "lowKcalDays": [], "daysCount": 0}
    if not weekly:
        return result
    items = weekly.get("items") if isinstance(weekly.get("items"), list) else []
    result["daysCount"] = len(items)
    required = {"早餐", "午餐", "晚餐"}
    for item in items:
        if not isinstance(item, dict):
            continue
        day = item.get("day", "")
        meals = item.get("meals") if isinstance(item.get("meals"), list) else []
        names = {meal.get("mealName") for meal in meals if isinstance(meal, dict)}
        missing = sorted(required - names)
        if missing:
            result["missingMeals"].append({"day": day, "missing": missing})
        if normalize_number(item.get("dailyTotalKcal")) and normalize_number(item.get("dailyTotalKcal")) < 1200:
            result["lowKcalDays"].append({"day": day, "dailyTotalKcal": item.get("dailyTotalKcal")})
    return result
...
def render_meal_completeness(final_plan):
    completeness = meal_plan_completeness(final_plan)
    problems = []
    if completeness["daysCount"] != 7:
        problems.append(f"菜谱天数为 {completeness['daysCount']}，期望 7 天")
    for item in completeness["missingMeals"]:
        problems.append(f"第{item['day']}天缺少：{','.join(item['missing'])}")
    for item in completeness["lowKcalDays"]:
        problems.append(f"第{item['day']}天热量偏低：{item['dailyTotalKcal']} kcal")
    problem_html = "<p class=\"pill good\">食谱完整性未发现结构问题</p>" if not problems else "<pre>" + escape(problems) + "</pre>"
    return (
        "<h3>食谱完整性</h3>"
        f"{problem_html}"
        f"{render_dietitian_meal_plan(final_plan)}"
    )
...
def render_output_quality(final_plan, summary):
    ...
    cards = [
        ...
        ("7天菜谱", completeness["daysCount"], "weeklyMealPlan 天数"),
        ("缺餐问题", len(completeness["missingMeals"]), "早餐/午餐/晚餐"),
        ("低热量天数", len(completeness["lowKcalDays"]), "dailyTotalKcal < 1200"),
        ("汇总错误", len((summary or {}).get("mismatches") or []), "营养明细求和"),
        ("0碳水食物", len((summary or {}).get("zeroCarbFoods") or []), "供人工复核"),
    ]
    weekly_note = "已生成 weeklyMealPlan" if weekly else "未生成 weeklyMealPlan"
    return (
        "<section class=\"panel\" id=\"output-quality\"><div class=\"section-head\">"
        "<div><p class=\"eyebrow\">OUTPUT</p><h2>干预方案产出质量</h2></div>"
        f"<p>{escape(weekly_note)}</p></div>"
        f"{render_metric_cards(cards)}"
        "<h3>分组内容质检</h3>"
        f"{render_plan_groups_detail(final_plan)}"
        f"{render_meal_completeness(final_plan)}"
        "<h3>食物明细异常复核</h3>"
...
<head>
<meta charset="utf-8">
<title>{escape(str(metadata.get('patientName', '未知患者')))} · 饮食方案测试结果</title>
<style>
...
.meal-subsection {{ margin-top:16px; }}
.meal-subsection > h4 {{ margin:0 0 10px; font-size:17px; }}
.nutrition-badges {{ display:grid; grid-template-columns:repeat(auto-fit,minmax(150px,1fr)); gap:8px; margin:10px 0; }}
.nutrition-badges span {{ border:1px solid #dfe6ed; border-radius:8px; padding:10px 12px; background:#fbfcfd; color:#526575; }}
.nutrition-badges b {{ color:#17202a; font-size:18px; }}
.dietitian-notes {{ display:flex; flex-wrap:wrap; gap:8px; margin:8px 0 16px; }}
.dietitian-notes span {{ background:#eef5f7; border:1px solid #d8e8ed; border-radius:999px; padding:6px 10px; color:#294f5a; font-size:13px; }}
.day-card {{ border:1px solid #d9e2ea; border-radius:8px; margin:12px 0; overflow:hidden; }}
.day-card > summary {{ display:grid; grid-template-columns:minmax(0,1fr) auto; gap:12px; align-items:center; cursor:pointer; padding:13px 14px; background:#f7fafb; }}
.day-card > summary span strong {{ display:block; font-size:16px; }}
.day-card > summary span em {{ display:block; color:#687887; font-style:normal; font-size:13px; margin-top:3px; }}
.day-summary-metrics {{ display:flex; flex-wrap:wrap; justify-content:flex-end; gap:6px; align-items:center; max-width:520px; }}
.day-summary-metrics span {{ background:#eef5f7; border:1px solid #d8e8ed; border-radius:999px; padding:5px 8px; color:#294f5a; font-size:12px; white-space:nowrap; }}
.day-summary-metrics .summary-label {{ background:#fff; border-color:#cfd8e3; color:#526575; font-weight:700; }}
.day-summary-metrics b {{ color:#17202a; }}
.day-total,.meal-list {{ padding:12px 14px; }}
.day-total h4,.meal-list h4 {{ margin:0 0 8px; }}
.day-total p {{ margin:8px 0 0; color:#5e6c79; }}
.meal-card {{ border:1px solid #e3e9ef; border-radius:8px; margin:8px 0; overflow:hidden; }}
.meal-card > summary {{ display:grid; grid-template-columns:minmax(0,1fr) auto; gap:10px; cursor:pointer; padding:10px 12px; background:#fff; }}
.meal-card > summary strong {{ margin-right:8px; }}
.meal-card > summary em {{ color:#526575; font-style:normal; font-size:13px; }}
.meal-card h5 {{ margin:10px 12px 6px; }}
.food-table th,.food-table td {{ font-size:13px; }}
...
@media (max-width: 760px) {{ .section-head,.two-col {{ display:block; }} .day-card > summary {{ grid-template-columns:1fr; }} .day-summary-metrics {{ justify-content:flex-start; max-width:none; }} }}
</style>
</head>
<body>
<header>
...
def build_parser():
    parser = argparse.ArgumentParser(description="AIHcare 饮食方案 Dify Chatflow 运行测试工具")
