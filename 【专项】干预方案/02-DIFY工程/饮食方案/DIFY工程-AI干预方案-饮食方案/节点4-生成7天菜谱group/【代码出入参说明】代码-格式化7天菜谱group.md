...
- `text1To3: String(JSON) <= 生成1~3天菜谱.text`
- `text4To7: String(JSON) <= 生成4~7天菜谱.text`

## 入参

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `text1To3` | String(JSON) | 是 | 当前 yml 实际绑定字段，传入第1-3天菜谱 LLM 的整段 JSON 文本。 |
| `text4To7` | String(JSON) | 是 | 当前 yml 实际绑定字段，传入第4-7天菜谱 LLM 的整段 JSON 文本。 |
| `mealPlanGroup1To3` | String(JSON) | 否 | 兼容字段；当前 yml 不按该字段绑定。 |
| `mealPlanGroup4To7` | String(JSON) | 否 | 兼容字段；当前 yml 不按该字段绑定。 |
| `mealPlanGroup` | String(JSON) | 否 | 兼容旧版单节点 7 天菜谱输入；分段模式下通常不传。 |
| `text` | String(JSON) | 否 | 兼容旧版单节点 7 天菜谱文本；分段模式下通常不传。 |
| `llmText` | String(JSON) | 否 | 兼容字段，作用同 `text`。 |
...
- 每餐营养汇总以该餐 `foods[]` 明细求和结果为准，会覆盖 LLM 原始输出：
  `mealTotalKcal=sum(foods[].kcal)`、
  `mealTotalProteinG=sum(foods[].proteinG)`、
  `mealTotalFatG=sum(foods[].fatG)`、
  `mealTotalCarbsG=sum(foods[].carbsG)`。
- 每日营养汇总以当天 `meals[]` 汇总求和结果为准，会覆盖 LLM 原始输出：
  `dailyTotalKcal=sum(meals[].mealTotalKcal)`、
  `dailyTotalProteinG=sum(meals[].mealTotalProteinG)`、
  `dailyTotalFatG=sum(meals[].mealTotalFatG)`、
  `dailyTotalCarbsG=sum(meals[].mealTotalCarbsG)`。
